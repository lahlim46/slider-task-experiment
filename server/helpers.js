...
export const calculateScores = (game, round, sliders) => {
  ...
  let effort = 0;
  for (let i = 0; i < player.round.get("value").length; i++) {
    for (let j = 0; j < sliders.numColumns; j++) {
      if (player.round.get("value")[i][j] === sliders.correctPosition) {
        effort++;
      }
    }
  }
  player.round.set("value", [effort, effort / player.get("numSliders")]);
...
const accommodateTies = (game, round) => {
  ...
  for (let i = 0; i < game.players.length - 1; i++) {
    ...
    if (
      isTie === false
      && game.players[i].round.get("value")[1] === game.players[i + 1].round.get("value")[1]
    ) {
      isTie = true;
      ties[currentRank] = 1;
    } else if (
